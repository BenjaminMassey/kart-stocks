pub fn from_obs(
    ...
    update_state(&mut frame, state, llm_model, llm_training_data, ocr_engine);
}
...
fn update_state(
    ...
    if let Some(new_place) = crate::extract::get_placement(llm_model, llm_training_data, frame) {
        state.place = new_place;
    }
    state.first_item = crate::extract::get_first_item(llm_model, llm_training_data, frame);
    if state.first_item.is_some() {
        state.second_item = crate::extract::get_second_item(llm_model, llm_training_data, frame);
    } else {
